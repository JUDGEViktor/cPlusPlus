// Плюсы_2Лаба.cpp : Определяет точку входа для приложения.
//
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		int R = 255, G = 0, B = 0;
		for (list <coordinates>::iterator it = track.begin(); it != track.end(); it++) {
			hBrush = CreateSolidBrush(RGB(R, G, B));
			old_brush = (HBRUSH)SelectObject(hdc, hBrush);
			SelectObject(hdc, hBrush);
			Rectangle(hdc, (*it).x, (*it).y, (*it).x + HEIGHT, (*it).y - HEIGHT);
			G += 20;
			B += 20;
